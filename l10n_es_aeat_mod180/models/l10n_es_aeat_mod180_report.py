...
class L10nEsAeatMod180Report(models.Model):
    ...
    perceptor_ids = fields.One2many("l10n.es.aeat.mod180.report.perceptor", "report_id")
    ...
    def calculate(self):
        ...
        for rec in self:
            ...
            for line in rec.tax_line_ids.filtered(lambda x: x.field_number == 2).mapped(
                ...
                self.env["l10n.es.aeat.mod180.report.perceptor"].create(
                    {
                        "report_id": rec.id,
...
class L10nEsAeatMod180ReportPerceptor(models.Model):
    _name = "l10n.es.aeat.mod180.report.perceptor"
    _description = "AEAT 180 report perceptor"
    ...
    def _compute_amount(self):
        for rec in self:
            rec.base_retenciones = rec.move_line_id.credit
            if rec.retención < 0:
                rec.signo = "N"
            else:
                rec.signo = " "
            rec.porcentaje_retencion = rec.move_line_id.tax_ids.amount
            rec.cuota_retencion = (
